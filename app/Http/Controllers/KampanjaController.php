...
/*     Input::get('naslov');
 ...
 $fajlovi=array("slika","slika2","slika3");
    ...
    $slika[3]=$this->fetch('slika4');
     $slika[4]=$this->fetch('slika5');
      $slika[5]=$this->fetch('slika6');
        ...
        $array = array('naslov' => Input::get('naslov'), 'cilj' => Input::get('cilj'), 'kategorija'=>Input::get('kategorija'),'skupljeno' => Input::get('skupljeno') ,
             'lokacija' => Input::get('lokacija'),'datumisteka' => Input::get('datumisteka'),'text' => Input::get('text'),
         'opsirnije' => Input::get('opsirnije'),'boja' => Input::get('boja'),'pozadina' => Input::get('pozadina')

          );

  Log::info($video);
//Log::info($slika[0]);     Log::info($slika[1]);         Log::info($slika[2]);
if( !empty($slika[0]) ) $array['slika']=$slika[0];
if(!empty($slika[1])) $array['slika2']=$slika[1];
if(!empty($slika[2])) $array['slika3']=$slika[2];

if(!empty($slika[3])) $array['slika4']=$slika[3];
if(!empty($slika[4])) $array['slika5']=$slika[4];
if(!empty($slika[5])) $array['slika6']=$slika[5];

if(!empty($video)) $array['videourl']=$video;
...
//if(!empty(Input::file('video')))  $array['videourl']=Input::get('video');
 ...
 $today = date("j, n, Y");
    ...
    $slika=$this->fetch('slika');
           Log::info($today);
            Log::info($naslov);
        $array = array('slug'=>$slug, 'naslov' => $naslov, 'update' => Input::get('update'), 'avatar' => Auth::user()->slika,  'user_id' => Auth::user()->id,
             'datum' => date("Y-m-d"),'slika'=>$slika );
...
&& $imageFileType != "gif" && $imageFileType != "mp4" && $imageFileType != "3gp" && $imageFileType != "avi") {
        ...
        echo "Greska u slanju slike.\n";
          Log::info("greska");
            ...
            public function uplatesnimi() {
               ...
               Main::whereid(Input::get('id') )->update([ 'uputstvo' => Input::get('text') ]);
          ...
          //   return Redirect::back();
             ...
             public function podrzi() {
                       if(Auth::user()->grad!="" && Input::get('lat')!="")
                           {
                          $array = array('slug'=>Input::get('slug'), 'ime_suportera' => Input::get('ime_suportera'),     'grad'=>Auth::user()->grad,    'lat'=>Input::get('lat'),    'long'=>Input::get('long'),
                          'slika' => Input::get('slika') );
                    }
                    else
                           {
                          $array = array('slug'=>Input::get('slug'), 'ime_suportera' => Input::get('ime_suportera'),
                          'slika' => Input::get('slika') );
                    }
...
static public function slugify($string, $delimiter = '-')
{
    ...
    return mb_strtolower($string);
                  ...
                  public function pokreni() {
                          ...
                          $array = array('slug'=>$slug, 'naslov' => Input::get('naslov'),

                          'cilj' => $cilj, 'lokacija' => Input::get('lokacija') , 'kategorija' => Input::get('kategorija'), 'objavio' => Auth::user()->fullname );

            $getid= Main::insertGetId($array);
             Podesavanja::insert(["kampanja_id" => $getid]);
 ...
 */
            ...
            Podesavanja::wherekampanja_id(Input::get('id') )->update([ $opcija => $odabir ]);
          ...
          Updates::whereid(Input::get('id') )->update([ 'update' => Input::get('update') ]);
         ...
         Main::whereid(Input::get('id') )->update([ Input::get('imetaga') => "" ]);
      ...
      if (Request::ajax()) {
                    return \Response::json(View::make('kampanje.lokacija', array('project' => $project,'podrska'=>$podrska))->render());
...
Session::set('loaded_komentari', 12);
   //     $komentari_objave = Komentariobjava::with('updates')->orderBy('id', 'DESC')->whereslug($slug)->get();
      ...
      if (Request::ajax()) {
                    return \Response::json(View::make('kampanje.pocetna', array('project' => $project, 'last_update'=> $last_update, 'updates'=>$updates,'komentari'=>$komentari,'podrska'=>$podrska))->render());
...
//Session::set('loaded_komentari', 12);
  ...
  //      $donatori = Donatori::orderBy('iznos', 'DESC')->whereslug($slug)->get();
     ...
     //  $podrska1 =json_encode( $podrska );
      ...
      if (Request::ajax()) {
                    return \Response::json(View::make('kampanje.updates', array('project' => $project,  'updates'=>$updates))->render());
...
Session::set('loaded_komentari', 12);
  ...
  //      $donatori = Donatori::orderBy('iznos', 'DESC')->whereslug($slug)->get();
   ...
   //    $podrska1 =json_encode( $podrska );
      ...
      if (Request::ajax()) {
                    return \Response::json(View::make('kampanje.comments', array('project' => $project, 'komentari'=>$komentari))->render());
...
//Session::set('loaded_komentari', 12);
      ...
      //  $donatori = Donatori::orderBy('iznos', 'DESC')->whereslug($slug)->get();
         $podrska = Podrska::whereslug($slug)->orderBy('id','ASC')->take(80)->get();
      ...
      if (Request::ajax()) {
                    return \Response::json(View::make('kampanje.podrzali', array('project' => $project, 'podrska'=>$podrska))->render());
