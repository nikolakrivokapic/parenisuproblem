...
if (Request::ajax()) {
            return \Response::json(View::make('posts', array('projects' => $projects))->render());
        }
...
$projects = Main::wherekategorija($kategorija)->orderBy('id', 'DESC')->wherelive('da')->paginate(9);
$count =   Main::count();
...
$projects = Main::wherelokacija($lokacija)->wherelive('da')->orderBy('id', 'DESC')->paginate(9);
$count =   Main::count();
if(Auth::check())   {
    $user=Auth::user();
    $kampanje = Main::whereobjavio($user->fullname)->get();
...
$projects = Main::whereoznaka($oznaka)->orderBy('id', 'DESC')->wherelive('da')->paginate(9);
$count =   Main::count();
     if (Request::ajax()) {
            return \Response::json(View::make('posts', array('projects' => $projects))->render());
        }
...
->orWhere('sender', '=', Auth::user()->id);
 ...
 return view('users.home', compact('projects','count','user','kampanje','brojneprocitanih'));
    ...
    {
         $projects = Main::orderBy('lokacija', 'ASC')->wherelive('da')->get(array('lokacija'));
