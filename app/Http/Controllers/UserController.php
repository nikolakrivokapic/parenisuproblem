...
class UserController extends Controller
{
    ...
    public function poruke($id)
    {
        ...
        $poruke = Poruke::with('poruke1')->where('receiver', $id)
            ->orWhere('sender', $id)
            ->orderBy('id', 'DESC')
            ->take(5)
            ->get()
    ...
    public function poruke1($id, $fullname)
    {
        ...
        $porukenove = Poruke1::where(function ($query) use($id, $id1) {
            $query->where('receiver', '=', $id)->where('sender', '=', $id1);
    ...
    public function loadmore()
    {
        $poruke = Poruke1::where(function ($query) {
            $query->where('receiver', '=', Input::get('id1'))->where('sender', '=', Input::get('id'));
        })->orWhere(function ($query) {
            $query->where('receiver', '=', Input::get('id'))
                ->where('sender', '=', Input::get('id1'));
        })
            ->orderBy('id', 'DESC')
            ->take(Session::get('loaded') + 7)
            ->get();
